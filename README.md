...
- пользователь нажимает кнопку, кратко описывает проблему и получает приватный канал тикета;
...
Команда отправит панель поддержки с кнопкой тикета и Telegram поддержки `@elix_sup`.
...
После первого запуска бот напишет в логи `TICKET_PANEL_MESSAGE_ID`; добавь его в `.env`, чтобы после перезапуска бот редактировал это же сообщение, а не создавал новое.
